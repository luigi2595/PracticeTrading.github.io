...
import type {
  Bar, ContractSpec, ImportedSession, MAType, Position, PropRules, RestingOrder, Session, SessionSummary, Settings, Side, Trade,
} from '../lib/types';
import type { PatternLabSaveShape, SavePayload } from '../lib/persistence';
import { evalProp, PROP_PRESETS } from '../lib/propfirm';
import type { PropContext, PropStatus } from '../lib/propfirm';
import { initialPropEval, initialPatternLab, initialTicket, DEFAULT_LEVELS } from './state';
...
/** Assembles what lib/propfirm needs from the store. Only sessions recorded
 *  since the evaluation started count toward it. */
export function propContextOf(state: SimState, spec: ContractSpec): PropContext {
  const last = state.session ? lastPrice(state.session, state.cursor, state.sub) : 0;
  const open = state.position ? openPnl(state.position, last, spec) : 0;
  return {
    sessionNets: state.sessions.slice(state.propEval.startSessionIdx).map((x) => x.net),
    realizedToday: state.realized,
    openPnl: open,
    peakEquity: state.propEval.peakEquity,
    tradedToday: state.sessionTrades > 0,
  };
}

export function propStatusOf(state: SimState, spec: ContractSpec): PropStatus {
  return evalProp(state.settings.prop, propContextOf(state, spec));
}

/** Recomputes the running peak, then applies any breach: a drawdown breach
 *  fails the evaluation outright and flattens; a daily-loss breach just ends
 *  the day. Safe to call on every substep — it is a no-op when the rules are
 *  off or already breached. */
function applyPropRules(state: SimState, spec: ContractSpec): SimState {
  const rules = state.settings.prop;
  if (!rules.on) return state;

  const ctx = propContextOf(state, spec);
  const equity = rules.accountSize + ctx.sessionNets.reduce((a, n) => a + n, 0) + ctx.realizedToday + ctx.openPnl;

  let next = state;
  if (equity > state.propEval.peakEquity) {
    next = { ...next, propEval: { ...next.propEval, peakEquity: equity } };
  }
  if (next.propEval.breached) return next;

  const status = evalProp(rules, propContextOf(next, spec));

  if (status.breach?.kind === 'drawdown') {
    // Mark the breach first: doFlatten re-enters this function via
    // closePositionCase, and the `breached` early-return above is what stops
    // that from recursing.
    next = { ...next, propEval: { ...next.propEval, breached: true, breachReason: status.breach.reason } };
    if (next.position) next = doFlatten(next, spec);
    next = { ...next, playing: false, locked: true, lockReason: status.breach.reason };
    return withAnnounce(withToast(next, 'Evaluation failed — drawdown line hit.'), 'Evaluation failed. ' + status.breach.reason);
  }

  if (status.breach?.kind === 'dailyLoss' && !next.locked) {
    next = { ...next, playing: false, locked: true, lockReason: status.breach.reason };
    return withAnnounce(withToast(next, 'Daily loss limit hit — done for the day.'), 'Trading locked. ' + status.breach.reason);
  }

  if (!next.propEval.passed && status.passReady) {
    next = { ...next, propEval: { ...next.propEval, passed: true, passedAt: new Date().toISOString() } };
    return withAnnounce(withToast(next, 'Evaluation passed — every rule met.'), 'Evaluation passed. Every rule met.');
  }
...
function closePositionCase(state: SimState, exitPrice: number, reason: string, spec: ContractSpec): SimState {
  ...
  return applyPropRules(next, spec);
}
...
function openPositionCase(state: SimState, side: Side, spec: ContractSpec): SimState {
  ...
  // Gated on `prop.on`: turning the rules off is how you back out of a blown
  // evaluation and go back to ordinary practice. A stale breach from a past
  // evaluation should not lock trading forever just because the flag is still
  // sitting in state — it re-surfaces if the rules are turned back on.
  if (state.settings.prop.on && state.propEval.breached) {
    return withToast(state, 'This evaluation is over — the drawdown line was hit. Start a new evaluation to trade again.');
  }
  ...
  const propCap = state.settings.prop.on ? state.settings.prop.maxContracts : 0;
  if (propCap > 0 && qty > propCap) {
    return withToast(state, 'This evaluation caps you at ' + propCap + ' contract' + (propCap === 1 ? '' : 's') + '. Size is a rule here, not a preference.');
  }
...
function advanceSubstep(state0: SimState, now: string): SimState {
  ...
  // 3b. the evaluation's drawdown line is watched continuously, because an
  // intraday trailing drawdown can be breached by open P&L alone.
  work = applyPropRules(work, spec);
...
export type Action =
  ...
  | { type: 'SET_PROP_RULES'; patch: Partial<PropRules> }
  | { type: 'APPLY_PROP_PRESET'; key: string }
  | { type: 'RESET_PROP_EVAL' }
...
export function reducer(state: SimState, action: Action): SimState {
  switch (action.type) {
    ...
    case 'SET_PROP_RULES': {
      // Any hand edit means the numbers no longer match a preset.
      const touchesNumbers = Object.keys(action.patch).some((k) => k !== 'on');
      const prop: PropRules = {
        ...state.settings.prop,
        ...action.patch,
        ...(touchesNumbers && !('presetKey' in action.patch) ? { presetKey: 'custom' } : {}),
      };
      let next: SimState = { ...state, settings: { ...state.settings, prop } };
      // Turning the rules on mid-flight anchors the peak at today's equity so
      // the trailing line starts from where the account actually is.
      if (prop.on && !state.settings.prop.on) {
        const spec = specFromSettings(next.settings);
        const ctx = propContextOf(next, spec);
        const equity = prop.accountSize + ctx.sessionNets.reduce((a, n) => a + n, 0) + ctx.realizedToday + ctx.openPnl;
        next = {
          ...next,
          propEval: {
            ...next.propEval,
            startedAt: next.propEval.startedAt ?? new Date().toISOString(),
            peakEquity: Math.max(prop.accountSize, equity),
          },
        };
      }
      return next;
    ...
    case 'APPLY_PROP_PRESET': {
      const preset = PROP_PRESETS.find((x) => x.key === action.key);
      if (!preset) return state;
      const prop: PropRules = { ...state.settings.prop, ...preset.rules, presetKey: preset.key };
      return {
        ...state,
        settings: { ...state.settings, prop },
        propEval: { ...initialPropEval, startedAt: new Date().toISOString(), startSessionIdx: state.sessions.length, peakEquity: prop.accountSize },
        locked: false,
        lockReason: '',
      };
    }

    case 'RESET_PROP_EVAL': {
      return withToast({
        ...state,
        propEval: {
          ...initialPropEval,
          startedAt: new Date().toISOString(),
          startSessionIdx: state.sessions.length,
          peakEquity: state.settings.prop.accountSize,
        },
        locked: false,
        lockReason: '',
      }, 'New evaluation started. Drawdown line reset to ' + fmt$(state.settings.prop.accountSize - state.settings.prop.maxDrawdown, 0) + '.');
    }
    ...
    case 'LOAD_PERSISTED': {
      ...
      const saved = d.settings as unknown as Partial<Settings> | undefined;
      const settings: Settings = saved
        ? { ...state.settings, ...saved, prop: { ...state.settings.prop, ...(saved.prop || {}) } }
        : state.settings;
    ...
    case 'RESET_ALL': {
      return { ...state, trades: [], sessions: [], userLevels: [], patternLab: { ...initialPatternLab }, propEval: { ...initialPropEval } };
    }
