...
export interface Settings {
  ...
  autosave: boolean;
  useReal: boolean;
  /** Prop-firm evaluation rules. See lib/propfirm.ts. */
  prop: PropRules;
}

/** How the maximum-drawdown line behaves.
 *  - `static`: fixed at accountSize - maxDrawdown, never moves.
 *  - `intraday`: trails the highest equity ever touched, open P&L included.
 *  - `eod`:      trails the highest balance at a session close only. */
export type TrailingMode = 'static' | 'intraday' | 'eod';

export interface PropRules {
  /** Master switch. Off means none of this is enforced or shown. */
  on: boolean;
  /** Which preset the current numbers came from ('custom' once edited). */
  presetKey: string;
  accountSize: number;
  /** Profit needed to pass, in dollars. 0 disables the target. */
  profitTarget: number;
  /** Maximum drawdown in dollars. 0 disables the drawdown rule. */
  maxDrawdown: number;
  trailing: TrailingMode;
  /** Most firms freeze a trailing line once it reaches the starting balance. */
  trailLocksAtStart: boolean;
  /** Daily loss limit in dollars, counting open P&L. 0 disables. */
  dailyLossLimit: number;
  /** No single day may be more than this % of total profit. 0 disables. */
  consistencyPct: number;
  /** Minimum number of days with at least one trade. 0 disables. */
  minTradingDays: number;
  /** Hard cap on contracts per position. 0 means no cap. */
  maxContracts: number;
}
