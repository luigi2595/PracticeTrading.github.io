...
import { PROP_PRESETS, TRAILING_LABEL } from '../lib/propfirm';
import type { TrailingMode } from '../lib/types';
  ...
  return (
    <section className="panel" id="panel-settings">
      <div className="sim half">
        ...
        {can('propFirmRules') && (
        <div className="card" id="propRulesCard">
          <h3>Prop firm evaluation</h3>
          <label className="switch">
            <input type="checkbox" checked={prop.on}
              onChange={(e) => dispatch({ type: 'SET_PROP_RULES', patch: { on: e.target.checked } })} />
            {' '}Enforce evaluation rules while I trade
          </label>
          <p className="hint" style={{ margin: '8px 0 0' }}>
            Most people who fail a funded-account evaluation do it on a rule — a trailing drawdown, a daily loss limit, a
            consistency requirement — rather than on their reading of the chart. Turn this on and the sim enforces the
            same rules, so the expensive lesson happens here instead of on a paid attempt.
          </p>
          <p className="hint" style={{ margin: '8px 0 0' }}>
            <b>This is a practice simulation only.</b> It is not affiliated with, endorsed by, or sponsored by any
            proprietary trading firm. Nothing here is a real funded account, no money changes hands, and no result shown
            here is submitted to or recognized by any evaluation program. Passing the checks in this simulator does not
            mean you will pass — or are likely to pass — a real firm's evaluation, whose actual rules you should always
            confirm directly with that firm before paying for an attempt.
          </p>

          {prop.on && (
            <>
              <div style={{ marginTop: 12 }}>
                <label className="lbl" htmlFor="propPreset">Start from</label>
                <select id="propPreset" value={prop.presetKey}
                  onChange={(e) => dispatch({ type: 'APPLY_PROP_PRESET', key: e.target.value })}>
                  {PROP_PRESETS.map((pr) => <option key={pr.key} value={pr.key}>{pr.label}</option>)}
                  <option value="custom">Custom — my own numbers</option>
                </select>
                <p className="hint" style={{ margin: '6px 0 0' }}>
                  {PROP_PRESETS.find((pr) => pr.key === prop.presetKey)?.blurb
                    ?? 'Your own numbers. Nothing here is tied to a particular firm.'}
                </p>
              </div>

              <div className="grid2" style={{ marginTop: 12 }}>
                <div>
                  <label className="lbl" htmlFor="propSize">Account size ($)</label>
                  <input id="propSize" type="number" step={5000} value={prop.accountSize}
                    onChange={(e) => dispatch({ type: 'SET_PROP_RULES', patch: { accountSize: parseFloat(e.target.value) || 0 } })} />
                </div>
                <div>
                  <label className="lbl" htmlFor="propTarget">Profit target ($)</label>
                  <input id="propTarget" type="number" step={500} value={prop.profitTarget}
                    onChange={(e) => dispatch({ type: 'SET_PROP_RULES', patch: { profitTarget: parseFloat(e.target.value) || 0 } })} />
                </div>
                <div>
                  <label className="lbl" htmlFor="propDD">Max drawdown ($)</label>
                  <input id="propDD" type="number" step={250} value={prop.maxDrawdown}
                    onChange={(e) => dispatch({ type: 'SET_PROP_RULES', patch: { maxDrawdown: parseFloat(e.target.value) || 0 } })} />
                </div>
                <div>
                  <label className="lbl" htmlFor="propTrail">Drawdown type</label>
                  <select id="propTrail" value={prop.trailing}
                    onChange={(e) => dispatch({ type: 'SET_PROP_RULES', patch: { trailing: e.target.value as TrailingMode } })}>
                    {(['static', 'eod', 'intraday'] as TrailingMode[]).map((m) => (
                      <option key={m} value={m}>{TRAILING_LABEL[m]}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="lbl" htmlFor="propDaily">Daily loss limit ($)</label>
                  <input id="propDaily" type="number" step={100} value={prop.dailyLossLimit}
                    onChange={(e) => dispatch({ type: 'SET_PROP_RULES', patch: { dailyLossLimit: parseFloat(e.target.value) || 0 } })} />
                </div>
                <div>
                  <label className="lbl" htmlFor="propConsistency">Consistency (% of profit, 0 = off)</label>
                  <input id="propConsistency" type="number" step={5} min={0} max={100} value={prop.consistencyPct}
                    onChange={(e) => dispatch({ type: 'SET_PROP_RULES', patch: { consistencyPct: parseFloat(e.target.value) || 0 } })} />
                </div>
                <div>
                  <label className="lbl" htmlFor="propDays">Minimum trading days</label>
                  <input id="propDays" type="number" step={1} min={0} value={prop.minTradingDays}
                    onChange={(e) => dispatch({ type: 'SET_PROP_RULES', patch: { minTradingDays: parseInt(e.target.value, 10) || 0 } })} />
                </div>
                <div>
                  <label className="lbl" htmlFor="propMaxCon">Max contracts (0 = no cap)</label>
                  <input id="propMaxCon" type="number" step={1} min={0} value={prop.maxContracts}
                    onChange={(e) => dispatch({ type: 'SET_PROP_RULES', patch: { maxContracts: parseInt(e.target.value, 10) || 0 } })} />
                </div>
              </div>

              <label className="switch" style={{ marginTop: 10 }}>
                <input type="checkbox" checked={prop.trailLocksAtStart} disabled={prop.trailing === 'static'}
                  onChange={(e) => dispatch({ type: 'SET_PROP_RULES', patch: { trailLocksAtStart: e.target.checked } })} />
                {' '}Trailing line stops once it reaches the starting balance
              </label>

              <div className="row wrap" style={{ marginTop: 12 }}>
                <button className="btn sm" onClick={() => dispatch({ type: 'RESET_PROP_EVAL' })}>Start a new evaluation</button>
              </div>

              <p className="hint" style={{ marginTop: 10 }}>
                The presets are generic <i>shapes</i>, not any particular firm&apos;s published rulebook — programmes change
                their numbers, and practising against a stale copy teaches the wrong limits. Pick the shape that matches
                yours, then type in your firm&apos;s actual figures.
              </p>
            </>
          )}
        </div>
      ...
      <details style={{ marginTop: 12 }}>
        ...
        <div className="hint" style={{ marginTop: 8, fontSize: 13, lineHeight: 1.7 }}>
          ...
          <br /><br />
          <b>HYPOTHETICAL OR SIMULATED PERFORMANCE RESULTS HAVE CERTAIN INHERENT LIMITATIONS.</b> Unlike an actual performance record, simulated
          results do not represent actual trading. Since the trades have not actually been executed, the results may have under- or
          over-compensated for the impact of certain market factors, such as lack of liquidity. Simulated trading programs are also subject to
          ...
          achieve profit or losses similar to those shown. The price data here is not a record of real prices at all — it is generated by an
          algorithm, unless you have imported your own bars.
        </div>
      </details>
    </section>
  );
